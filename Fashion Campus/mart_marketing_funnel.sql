CREATE OR REPLACE TABLE `datascience-414913.DB_FMA.mart_marketing_funnel` AS
...
WITH session_events AS (
  SELECT
    ...
    COUNTIF(LOWER(TRIM(payment_status)) = 'paid')                              AS paid_sessions,
    SUM(CASE WHEN LOWER(TRIM(payment_status)) = 'paid' 
        THEN item_price * quantity ELSE 0 END)                                  AS session_revenue
  FROM `datascience-414913.DB_FMA.click_stream`
  GROUP BY 1,2,3
)
